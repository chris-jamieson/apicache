...
function ApiCache() {
  ...
  this.clear = async function(target, isAutomatic) {
    ...
    if (group) {
      ...
      group.forEach(async function(key) {
        ...
        if (!globalOptions.redisClient) {
          ...
          try {
            ...
            console.log(
              '[apicache] error in redis.del("' + key + '"): ',
              err.message
            );
          }
        }
      ...
      if (!redis) {
        ...
        try {
          ...
          console.log(
            '[apicache] error in redis.del("' + target + '")',
            err.message
          );
        }
      }
      ...
      if (!redis) {
        ...
        index.all.forEach(async function(key) {
          clearTimeout(timers[key]);
          delete timers[key];
          try {
            if (redis && redis.isReady) {
